  ...
  return (
    <div className="overflow-auto -z-10">
      <div className="flex justify-between mt-8 mx-4 flex-wrap gap-3 font-poppins">
        <div className="flex justify-start gap-3 flex-wrap">
          <div className="relative inline-block">
            <button
              className={`text-lg py-1.5 w-36 ${activeTab === "tab1" ? "text-white bg-gradient-to-r from-[#3D03FA] to-[#A71CD2]" : "text-black bg-white"}`}
              onClick={() => setActiveTab("tab1")}
            >
              All Courses
            </button>
            <div className="absolute -top-8 -right-10 w-8 h-8 bg-[#110038] transform rotate-45 -translate-x-1/2 translate-y-1/2"></div>
          </div>
          <div className="relative inline-block">
            <button
              className={`text-lg py-1.5 w-36 ${activeTab === "tab2" ? "text-white bg-gradient-to-r from-[#3D03FA] to-[#A71CD2]" : "text-black bg-white"}`}
          ...
          <div className="relative inline-block">
            <button
              className={`text-lg py-1.5 w-36 ${activeTab === "tab3" ? "text-white bg-gradient-to-r from-[#3D03FA] to-[#A71CD2]" : "text-black bg-white"}`}
      ...
      <div className="mx-1 overflow-auto">
        {activeTab === "tab1" && <AllCourse courses={filteredCourses} handleCourse={handleCourse} handleCertificate={handleCertificate} />}
      </div>
      <div className="mx-1">
        {activeTab === "tab2" && <AllCourse courses={completed.filter(course => course.mainTopic.toLowerCase().includes(searchQuery.toLowerCase()))} handleCourse={handleCourse} handleCertificate={handleCertificate} />}
      </div>
      <div className="mx-1">
        {activeTab === "tab3" && <AllCourse courses={active.filter(course => course.mainTopic.toLowerCase().includes(searchQuery.toLowerCase()))} handleCourse={handleCourse} handleCertificate={handleCertificate} />}
      </div>
    </div>
  );
};
